...
export function PrintDinnerPage() {
  ...
  return <main className="print-page dinner-packet"><PrintActions /><header><p>Service · dinner packet</p><h1>{data.dinner.title}</h1><div><span>{format(parseISO(`${data.dinner.event_date}T12:00:00`), 'EEEE, MMMM d, yyyy')} at {data.dinner.serve_time.slice(0, 5)}</span><span>{data.dinner.guest_count} guests · {data.dinner.cuisine}</span></div></header><section><h2>Menu</h2><ol>{data.recipes.map((recipe) => <li key={recipe.id}>{recipe.short_title || recipe.translated_title || recipe.title} — {recipe.target_servings || data.dinner.guest_count} servings</li>)}</ol></section><section className="print-break"><h2>Shopping list</h2><div className="print-shopping">{data.shopping.filter((item) => !item.purchased).map((item) => { const components = remainingComponents(item); return <div key={item.id}><span>□ {item.item}{item.notes && <small>{item.notes}</small>}</span><strong>{components.length ? components.map((part) => `${formatQuantity(part.quantity)} ${part.unit}`).join(' + ') : `${formatQuantity(remainingQuantity(item))} ${item.unit}`}</strong></div>; })}</div></section><section className="print-break"><h2>Prep timeline</h2>{days.map((day) => <div className="print-day" key={day}><h3>{taskDayLabel(data.dinner, day)}</h3>{sortTimeline(data.tasks.filter((task) => task.day_offset === day)).map((task) => <div key={task.id}><time>{task.start_time.slice(0, 5)}</time><span>□ {task.title}{task.notes && <small>{task.notes}</small>}</span><small>{task.duration_minutes} min · {task.resource}</small></div>)}</div>)}</section>{data.recipes.map((recipe) => { const scale = recipeScale(recipe, data.dinner.guest_count); return <section className="print-break print-recipe" key={recipe.id}><h2>{recipe.short_title || recipe.translated_title || recipe.title}</h2><p>{recipe.target_servings || data.dinner.guest_count} servings · {scale.toFixed(2)}× source</p><h3>Ingredients</h3><ul className="print-ingredients">{data.ingredients.filter((row) => row.recipe_id === recipe.id).map((ingredient) => <li key={ingredient.id}><strong>{ingredient.quantity == null ? 'As needed' : `${formatQuantity(ingredient.quantity * scale)} ${ingredient.unit}`}</strong><span>{ingredient.translated_text || ingredient.item || ingredient.raw_text}</span></li>)}</ul><h3>Method</h3><ol className="print-steps">{data.steps.filter((row) => row.recipe_id === recipe.id).map((step) => <li key={step.id}>{step.translated_text || step.raw_text}</li>)}</ol></section>; })}</main>;
}
