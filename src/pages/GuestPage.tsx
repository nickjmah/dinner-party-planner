...
export function GuestPage() {
  ...
  return <div className="guest-page"><header className="guest-header"><div className="brand-mark"><ChefHat /><span>Service</span></div><p>Shared by your host · Read only</p></header><main><section className="guest-hero"><div><p className="eyebrow">{data.dinner.cuisine || 'Dinner party'}</p><h1>{data.dinner.title}</h1><div className="guest-meta"><span><CalendarDays />{format(parseISO(`${data.dinner.event_date}T12:00:00`), 'EEEE, MMMM d')}</span><span><Clock />{data.dinner.serve_time.slice(0, 5)}</span><span><Users />{data.dinner.guest_count} guests</span></div></div></section><section><p className="eyebrow">The menu</p><div className="guest-menu">{data.recipes.map((recipe, index) => <Card key={`${recipe.title}-${index}`}><h2>{recipe.short_title || recipe.translated_title || recipe.title}</h2><p>{recipe.target_servings || data.dinner.guest_count} planned servings</p></Card>)}</div></section><section className="guest-columns"><div><h2><ShoppingBasket />Remaining shopping</h2>{remaining.length ? <ul className="guest-list">{remaining.map((item, index) => <li key={`${item.item}-${index}`}><span>{item.item}{item.notes && <small>{item.notes}</small>}</span><strong>{item.component_requirements.length ? item.component_requirements.map((part) => `${formatQuantity(part.quantity)} ${part.unit}`).join(' + ') : `${formatQuantity(item.quantity)} ${item.unit}`}</strong></li>)}</ul> : <Card className="guest-complete"><Check />Shopping is complete</Card>}</div><div><h2><CookingPot />Prep plan</h2>{Object.entries(tasksByDay).sort(([a], [b]) => Number(a) - Number(b)).map(([day, tasks]) => <div className="guest-day" key={day}><h3>{taskDayLabel(data.dinner, Number(day))}</h3>{tasks.map((task, index) => <div className="guest-task" key={`${task.title}-${index}`}><time>{task.start_time.slice(0, 5)}</time><div><strong>{task.title}</strong><span>{task.duration_minutes} min · {task.resource}</span>{task.notes && <small>{task.notes}</small>}</div></div>)}</div>)}</div></section></main></div>;
}
